...
namespace BEPUphysics.Entities
{
        ...
        IEquatable<Entity>
    {
        ...
        void IForceUpdateable.UpdateForForces(Fix64 dt)
        {
            ...
            if (linear > F64.C0)
            {
                Vector3.Multiply(ref linearVelocity, TrueSync.TSMath.Pow(MathHelper.Clamp(F64.C1 - linear, F64.C0, F64.C1), dt), out linearVelocity);
            }
            ...
            {
#if CONSERVE
                Vector3.Multiply(ref angularMomentum, TrueSync.TSMath.Pow(MathHelper.Clamp(1 - angular, 0, 1), dt), out angularMomentum);
#else
				Vector3.Multiply(ref angularVelocity, TrueSync.TSMath.Pow(MathHelper.Clamp(F64.C1 - angular, F64.C0, F64.C1), dt), out angularVelocity);
#endif
            }
