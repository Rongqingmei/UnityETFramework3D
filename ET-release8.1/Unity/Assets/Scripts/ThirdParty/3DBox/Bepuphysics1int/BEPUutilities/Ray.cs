...
namespace BEPUutilities
{
    ...
    public struct Ray
    {
        ...
        public bool Intersects(ref BoundingBox boundingBox, out TrueSync.FP t)
        {
			TrueSync.FP tmin = F64.C0, tmax = TrueSync.FP.MaxValue;
            if (TrueSync.FP.Abs(Direction.X) < Toolbox.Epsilon)
            {
            ...
            if (TrueSync.FP.Abs(Direction.Y) < Toolbox.Epsilon)
            {
            ...
            if (TrueSync.FP.Abs(Direction.Z) < Toolbox.Epsilon)
            {
        ...
        /// <param name="t">The length along the ray to the impact, if any impact occurs.</param>
        /// <returns>True if the ray intersects the target, false otherwise.</returns>
        public bool Intersects(BoundingBox boundingBox, out TrueSync.FP t)
        {
        ...
        /// <param name="t">The length along the ray to the impact, if any impact occurs.</param>
        /// <returns>True if the ray intersects the target, false otherwise.</returns>
        public bool Intersects(ref Plane plane, out TrueSync.FP t)
        {
			TrueSync.FP velocity;
            Vector3.Dot(ref Direction, ref plane.Normal, out velocity);
            if (TrueSync.FP.Abs(velocity) < Toolbox.Epsilon)
            {
                t = F64.C0;
                return false;
            }
			TrueSync.FP distanceAlongNormal;
        ...
        /// <param name="t">The length along the ray to the impact, if any impact occurs.</param>
        /// <returns>True if the ray intersects the target, false otherwise.</returns>
        public bool Intersects(Plane plane, out TrueSync.FP t)
        {
        ...
        public void GetPointOnRay(TrueSync.FP t, out Vector3 v)
        {
