...
namespace BEPUutilities
{
    ...
    public static partial class ConvexHullHelper
    {
        ...
        {
            TrueSync.FP maximumDot = -TrueSync.FP.MaxValue;
            int extremeIndex = 0;
        ...
        private static void GetExtremePoints(ref Vector3 direction, RawList<Vector3> points, out TrueSync.FP maximumDot, out TrueSync.FP minimumDot, out int maximumIndex, out int minimumIndex)
        {
            maximumIndex = 0;
            minimumIndex = 0;
        ...
        private static void ComputeInitialTetrahedron(RawList<Vector3> points, RawList<int> outsidePointCandidates, RawList<int> triangleIndices, out Vector3 centroid)
        {
            ...
            TrueSync.FP minimumX = TrueSync.FP.MaxValue, maximumX = -TrueSync.FP.MaxValue;
            ...
            TrueSync.FP minimumDot, maximumDot;
            ...
            if (TrueSync.FP.Abs(dot - minimumDot) > TrueSync.FP.Abs(dot - maximumDot))
            {
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                if (TrueSync.FP.Abs(volume) < Toolbox.BigEpsilon)
                {
                    throw new ArgumentException("Point set is degenerate; convex hulls must have volume.");
                }
                if (volume < F64.C0)
